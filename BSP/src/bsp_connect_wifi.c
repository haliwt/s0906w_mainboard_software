	...
	if(g_wifi.gwifi_link_net_success==0 && g_pro.gpower_on == power_off){
     ...
     if(g_wifi.gwifi_link_net_success==1  && dc_power_on ==0){
          ...
          if(g_pro.gpower_on == power_off){
             ...
             tx_thread_sleep(20);//HAL_Delay(50);
...
static void Auto_SmartPhone_TryToLink_TencentCloud(void)
{
  ...
  if(power_on_login_tencent_cloud_flag==2 && g_wifi.gTimer_link_net_timer_time >3){
   ...
   if(g_wifi.gwifi_link_net_success==1 && power_on_login_tencent_cloud_flag ==4){
            ...
            power_on_login_tencent_cloud_flag++;
            if(g_pro.disp_second_f ==1){
				SendWifiData_To_Cmd(0x1F,0x01); 
				tx_thread_sleep(10);
			}//link wifi order 1 --link wifi net is success.
	}
    else if(g_wifi.gwifi_link_net_success ==0 && power_on_login_tencent_cloud_flag ==4){
       power_on_login_tencent_cloud_flag++;
        if(g_pro.disp_second_f ==1){SendWifiData_To_Cmd(0x1F,0x00);
	    tx_thread_sleep(10);
        	}
    }
}
...
{
   static uint8_t beijing_step,auto_link_net_flag;
   uint8_t real_hours,real_minutes,real_seconds;
    
   static uint16_t flag_switch;
    ...
    switch(g_wifi.wifi_get_beijing_step){
     ...
     case 0: //WT.EDIT .2024.08.10
      ...
      if(g_wifi.gwifi_link_net_success==1 && g_wifi.gTimer_get_data_from_tencent_data > 9){ ////9
        ...
        if(flag_switch > 400 && g_pro.gpower_on == power_on){//20ms
             flag_switch=0;
        ...
        else if(g_pro.gpower_on == power_off && flag_switch > 400){ //9*4 =36s.
            flag_switch=0;
            g_wifi.get_rx_beijing_time_enable=0;
            Update_Dht11_Totencent_Value();
            tx_thread_sleep(20);//HAL_Delay(200) //WT.EDIT 2024.08.10
    ...
    break;
     ...
     case 3:
         ...
         case 0:
    	    ...
    	    tx_thread_sleep(30);//HAL_Delay(20); //WT.EDIT .2024.08.10//HAL_Delay(20);
    	    beijing_step =1;
         ...
         case 1:
        	    ...
        	    tx_thread_sleep(20);//HAL_Delay(20); //WT.EDIT .2024.08.10
