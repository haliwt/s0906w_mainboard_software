...
void Tencent_Cloud_Rx_Handler(void)
{
	...
	if(strstr((char *)g_wifi.wifi_rx_data_array,"ptc\":0")){
            if(g_pro.gpower_on ==power_on){
                 ...
                 g_pro.gDry = 0;
				 LED_DRY_OFF();
			     g_pro.g_manual_shutoff_dry_flag =1;//WT.EDIT 2026.01.27
   ...
   switch(gl_msg.response_wifi_signal_label){
	...
	case OPEN_ON_ITEM:
	  ...
	  case PTC_OFF_ITEM:
		 ...
		 tx_thread_sleep(300);//HAL_Delay(350);
		 g_pro.gDry = 0;
		 LED_DRY_OFF();
	     DRY_CLOSE();
		 g_pro.g_manual_shutoff_dry_flag =1;//WT.EDIT 2026.01.27
	  ...
	  case APP_TIMER_POWER_ON_REF :
		   ...
		   if(strstr((char *)TCMQTTRCVPUB,"open\":1")){
			  ...
			  g_wifi.app_timer_power_on_flag = 1;
			   ...
			   if(g_pro.disp_second_f ==1){SendWifiData_To_Cmd(0x21,0x01); //smart phone is open that App timer 
               tx_thread_sleep(10);
			   	}
			    g_wifi.gwifi_link_net_success =1;
			...
			if(g_pro.disp_second_f ==1){SendWifiData_To_Cmd(0x21,0x0); //turn off power off
			tx_thread_sleep(10);//HAL_Delay(10);
				}
